...
import com.dscs.tools.imageselecttools.adapter.FolderAdapter;
import com.dscs.tools.imageselecttools.adapter.ImageGridAdapter;
import com.dscs.tools.imageselecttools.bean.Folder;
import com.dscs.tools.imageselecttools.bean.Image;
import com.dscs.tools.utils.FileUtils;
import com.dscs.tools.utils.ScreenUtils;
...
public class MultiImageSelectorFragment extends Fragment {
    ...
    public static final String TAG = "show_MultiImageSelectorFragment";
    public final static String CROP_PATH = "/DCIM/Screenshots";
    private static final int REQUEST_STORAGE_WRITE_ACCESS_PERMISSION = 110;
    private static final int REQUEST_CAMERA = 100;
    private static final int REQUEST_CROP = 120;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mContrxt = this.getActivity();
        selector = (MultiImageSelector) getArguments().getSerializable("Serializable");
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CAMERA) {
            if (resultCode == Activity.RESULT_OK) {
                if (mTmpFile != null) {
                    if (selector.mCorp) {
                        cropPicture(mContrxt, mTmpFile.toString());
                    } else {
                        if (mCallback != null) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                if (mCallback != null) {
                    imageFile = new File(data.getStringExtra("path"));
                    mCallback.onCameraShot(imageFile);
                }
            } else {
                makeText(mContrxt, "图片获取异常", LENGTH_SHORT).show();
            }
        }
    }
            ...
            if (intent.resolveActivity(getActivity().getPackageManager()) != null) {
                ...
                if (mTmpFile != null && mTmpFile.exists()) {
                    ...
                    makeText(getActivity(),
                            R.string.mis_error_image_not_exist,
                            LENGTH_SHORT).show();
                }
            } else {
                makeText(getActivity(), R.string.mis_msg_no_camera, LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void requestPermission(final String permission,
        ...
        if (shouldShowRequestPermissionRationale(permission)) {
            new AlertDialog.Builder(getContext())
                    ...
                    .setPositiveButton(R.string.mis_permission_dialog_ok,
            ...
            if (mode == MODE_MULTI) {
                if (resultList.contains(image.path)) {
                    ...
                    if (selectImageCount() == resultList.size()) {
                        makeText(getActivity(), R.string.mis_msg_amount_limit, LENGTH_SHORT).show();
                        return;
                    }
                ...
                if (mCallback != null) {
                    if (selector.mCorp) {
                        cropPicture(mContrxt, image.path);
                    } else {
    ...
    public void cropPicture(Activity activity, String path) {
        T.corp().params(path,1,selector,true).start(this);
    }
}
